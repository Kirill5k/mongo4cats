...
class MongoDbClientSpec extends AnyWordSpec with Matchers with MongoEmbedded {
  ...
  "A MongoDbClient" should {
    "connect to a db via connection string" in {
      ...
      withRunningMongoEmbedded() {
        val result = MongoDbClient.mongoDbClient[IO](connectionString)
    ...
    "connect to a db via server address string" in {
      withRunningMongoEmbedded(port = 54321) {
        val result = MongoDbClient.mongoDbClient[IO](MongoServerAddress("localhost", 54321))
