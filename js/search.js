...
function prepareIdxAndDocMap() {
  const docs = [  
    ...
    } ,    
    {
      ...
      "url": "/mongo4cats/docs/",
      "content": "Getting Started Dependencies In order to begin, the following dependency needs to be added to your build.sbt: libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-core\" % \"&lt;version&gt;\" For automatic derivation of Bson codecs via Circe, add this: libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-circe\" % \"&lt;version&gt;\" For automatic derivation of Bson codecs via ZIO Json, add this: libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-zio-json\" % \"&lt;version&gt;\" For the ability to use embedded MongoDB in your tests, add this: libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-embedded\" % \"&lt;version&gt;\" % Test For ZIO 2 integration, add this: libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-zio\" % \"&lt;version&gt;\" libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-zio-embedded\" % \"&lt;version&gt;\" Next steps Once everything is in place, you can start accessing your data: Making a connection Getting a collection Working with documents"
    } ,    
    {
      "title": "mongo4cats",
      "url": "/mongo4cats/",
      "content": "mongo4cats MongoDB Java client wrapper compatible with Cats-Effect/Fs2 and ZIO. Available for Scala 2.12, 2.13 and 3.2. Documentation is available in the documentation section. Dependencies Add this to your build.sbt (depends on cats-effect and FS2): libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-core\" % \"&lt;version&gt;\" libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-embedded\" % \"&lt;version&gt;\" % Test Alternatively, for ZIO 2, add this: libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-zio\" % \"&lt;version&gt;\" libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-zio-embedded\" % \"&lt;version&gt;\" % Test Optional support for circe or zio-json can be enabled with: // circe libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-circe\" % \"&lt;version&gt;\" // zio-json libraryDependencies += \"io.github.kirill5k\" %% \"mongo4cats-zio-json\" % \"&lt;version&gt;\" Quick start with Cats Effect import cats.effect.{IO, IOApp} import mongo4cats.client.MongoClient import mongo4cats.operations.{Filter, Projection} import mongo4cats.bson.Document import mongo4cats.bson.syntax._ object Quickstart extends IOApp.Simple { override val run: IO[Unit] = MongoClient.fromConnectionString[IO](\"mongodb://localhost:27017\").use { client =&gt; for { db &lt;- client.getDatabase(\"my-db\") coll &lt;- db.getCollection(\"docs\") _ &lt;- coll.insertMany((0 to 100).map(i =&gt; Document(\"name\" := s\"doc-$i\", \"index\" := i))) docs &lt;- coll.find .filter(Filter.gte(\"index\", 10) &amp;&amp; Filter.regex(\"name\", \"doc-[1-9]0\")) .projection(Projection.excludeId) .sortByDesc(\"name\") .limit(5) .all _ &lt;- IO.println(docs.mkString(\"[\\n\", \",\\n\", \"\\n]\")) } yield () } } Quick start with ZIO import mongo4cats.bson.Document import mongo4cats.bson.syntax._ import mongo4cats.operations.{Filter, Projection} import mongo4cats.zio.{ZMongoClient, ZMongoCollection, ZMongoDatabase} import zio._ object Zio extends ZIOAppDefault { val client = ZLayer.scoped[Any](ZMongoClient.fromConnectionString(\"mongodb://localhost:27017\")) val database = ZLayer.fromZIO(ZIO.serviceWithZIO[ZMongoClient](_.getDatabase(\"my-db\"))) val collection = ZLayer.fromZIO(ZIO.serviceWithZIO[ZMongoDatabase](_.getCollection(\"docs\"))) val program = for { coll &lt;- ZIO.service[ZMongoCollection[Document]] _ &lt;- coll.insertMany((0 to 100).map(i =&gt; Document(\"name\" := s\"doc-$i\", \"index\" := i))) docs &lt;- coll.find .filter(Filter.gte(\"index\", 10) &amp;&amp; Filter.regex(\"name\", \"doc-[1-9]0\")) .projection(Projection.excludeId) .sortByDesc(\"name\") .limit(5) .all _ &lt;- Console.printLine(docs.mkString(\"[\\n\", \",\\n\", \"\\n]\")) } yield () override def run = program.provide(client, database, collection) } If you find this library useful, consider giving it a ⭐!"
    } ,    
    {
      "title": "Index",
