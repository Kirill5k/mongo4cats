...
private[mongo4cats] object ContainerValueWriter {
  ...
  def writeBsonValue(
    ...
    bsonValue match {
      ...
      case BsonValue.BBinary(value, subtype) => writer.writeBinaryData(new BsonBinary(subtype, value))
      ...
      case BsonValue.BRegex(value, options)  => writer.writeRegularExpression(new BsonRegularExpression(value.pattern.pattern(), options))
    }
